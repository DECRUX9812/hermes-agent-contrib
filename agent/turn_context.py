...
def build_turn_context(
    ...
    # Bot Mode DM + mailbox tools — injected ONLY into a bot's canonical "Bot Chat"
    # session (same gate as the protocol section); gate is session-stable, so cache-safe.
    try:
        ...
        ensure_update_task_tool(agent)
    except Exception:
        logger.debug("bot-mode tool injection skipped", exc_info=True)
